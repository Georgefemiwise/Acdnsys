...
export default function EnhancedManagementPage() {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<"overview" | "users" | "plates">("overview");
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(`${API_BASE}/users?active_only=${!filters.showInactive}`);
  ...
  const fetchPlates = async () => {
    try {
      const response = await fetch(`${API_BASE}/plates?active_only=${!filters.showInactive}`);
  ...
  const handlePlateSubmit = async (plateData: any) => {
    setFormSubmitting(true);
    try {
      const url = editingPlate ? `${API_BASE}/plates/${editingPlate.id}` : `${API_BASE}/plates`;
      ...
      showSuccess(editingPlate ? "License plate updated successfully!" : "License plate registered successfully!");
  ...
  const handleDeleteUser = async (userId: number, hardDelete = false) => {
    ...
    try {
      ...
      showSuccess(`User ${hardDelete ? 'deleted' : 'deactivated'} successfully`);
      await fetchUsers();
  ...
  const handleDeletePlate = async (plateId: number, hardDelete = false) => {
    ...
    try {
      ...
      showSuccess(`License plate ${hardDelete ? 'deleted' : 'deactivated'} successfully`);
      await Promise.all([fetchUsers(), fetchPlates()]);
    ...
    return plates.filter(plate => {
      ...
      if (filters.searchTerm) {
        ...
        const matchesSearch = 
          ...
          (plate.owner_name && plate.owner_name.toLowerCase().includes(searchLower)) ||
          (plate.vehicle_make && plate.vehicle_make.toLowerCase().includes(searchLower)) ||
          (plate.vehicle_model && plate.vehicle_model.toLowerCase().includes(searchLower));
  ...
  const dashboardStats = useMemo(() => {
    ...
    const usersWithPlates = users.filter(u => u.plate_count && u.plate_count > 0).length;
    ...
    return {
      ...
      averagePlatesPerUser: users.length > 0 ? (plates.length / users.length).toFixed(1) : "0"
    };
  }, [users, plates]);
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto p-6">
        <LoadingSpinner size="lg" text="Loading management dashboard..." className="py-20" />
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="dropdown dropdown-end">
            ...
            <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
              <li><a><Download className="w-4 h-4" /> Export Data</a></li>
              <li><a><Upload className="w-4 h-4" /> Import Data</a></li>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="stat bg-base-100 shadow-lg rounded-xl">
              ...
              <div className="stat-value text-success">{dashboardStats.totalPlates}</div>
              <div className="stat-desc">
                {dashboardStats.primaryPlates} primary, {dashboardStats.secondaryPlates} secondary
              </div>
            </div>
            ...
            <div className="stat bg-base-100 shadow-lg rounded-xl">
              ...
              <div className="stat-value text-info">{dashboardStats.usersWithPlates}</div>
              <div className="stat-desc">
                {((dashboardStats.usersWithPlates / dashboardStats.totalUsers) * 100).toFixed(1)}% coverage
              </div>
            </div>
            ...
            <div className="stat bg-base-100 shadow-lg rounded-xl">
              ...
              <div className="stat-value text-warning">{dashboardStats.averagePlatesPerUser}</div>
              <div className="stat-desc">
              ...
              <div className="card-body">
                ...
                <div className="space-y-3">
                  {plates.slice(0, 5).map(plate => (
                    <div key={plate.id} className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${plate.is_active ? 'bg-success' : 'bg-error'}`} />
                        <div>
                          <div className="font-mono font-bold">{plate.plate}</div>
                          <div className="text-sm text-gray-600">{plate.owner_name}</div>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        {plate.is_primary && <Star className="w-4 h-4 text-warning" />}
                        <div className="badge badge-outline">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="dropdown dropdown-end">
                          ...
                          <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                            <li><a onClick={() => setSelectedUser(user)}><Eye className="w-4 h-4" /> View Details</a></li>
                            <li><a onClick={() => { setEditingUser(user); setShowUserForm(true); }}><Edit3 className="w-4 h-4" /> Edit</a></li>
                            <li><a onClick={() => handleDeleteUser(user.id)}><Trash2 className="w-4 h-4" /> Delete</a></li>
                          </ul>
                        </div>
                      </div>
                    </div>
                  ...
                  {filteredUsers.map(user => (
                    <tr key={user.id}>
                      <td>
                        ...
                        {user.address && <div className="text-sm opacity-50">{user.address}</div>}
                      </td>
                      <td>{user.phone}</td>
                      <td>{user.email || '-'}</td>
                      <td>
                        <div className="badge badge-outline">{user.plate_count || 0}</div>
                      </td>
                      <td>
                        {user.is_active ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              ...
              <select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, plateFilter: e.target.value }))}
              >
              ...
              <select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, vehicleFilter: e.target.value }))}
              >
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPlates.map(plate => (
                <div key={plate.id} className="card bg-base-100 shadow-lg hover:shadow-xl transition-shadow">
                  <div className="card-body">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                      ...
                      <div className="dropdown dropdown-end">
                        ...
                        <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                          <li><a onClick={() => { setEditingPlate(plate); setShowPlateForm(true); }}><Edit3 className="w-4 h-4" /> Edit</a></li>
                          <li><a onClick={() => handleDeletePlate(plate.id)}><Trash2 className="w-4 h-4" /> Delete</a></li>
                        </ul>
                      </div>
                    </div>
                  ...
                  {filteredPlates.map(plate => (
                    <tr key={plate.id}>
                      ...
                      <td>
                        ...
                        <div className="text-sm opacity-50">{plate.owner_phone}</div>
                      </td>
                      <td>
                        {plate.vehicle_make ? (
                          <div>
                            ...
                            {plate.vehicle_color && (
                              <div className="text-sm opacity-50">{plate.vehicle_color}</div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td>
                        <div className="flex gap-2">
                          <button
                            className="btn btn-ghost btn-sm"
                            onClick={() => { setEditingPlate(plate); setShowPlateForm(true); }}
                          >
                            <Edit3 className="w-4 h-4" />
                          </button>
                          <button
          ...
          {filteredPlates.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">No License Plates Found</h3>
              <p className="text-gray-500 mb-4">
